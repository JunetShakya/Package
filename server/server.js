...
const calculatePackages = (items) => {
  ...
  for (const item of items) {
    if (currentPackage.totalPrice + item.price > 250) {
      packages_one.push(currentPackage);
  ...
  packages_one.forEach((pkg) => {
    pkg.courierPrice =
      courierCharges.find((c) => pkg.totalWeight <= c.maxWeight)?.charge || 20;
  });
...
app.post("/api", (req, res) => {
  ...
  const packages = calculatePackages(sortedItems);
  res.json({ packages });
});
